...
def create_directory(path):
    ...
    if not os.path.exists(path):
        os.makedirs(path); logging.info(f"📁 Carpeta creada: {path}")
    else:
...
def validate_container(image_path, binary):
    ...
    try:
        ...
        logging.error(f"❌ {binary} NO disponible o contenedor no encontrado en: {image_path}"); exit(1)
...
def download_files(urls, destination_dir):
    ...
    for url in urls:
        ...
        if os.path.exists(gz_filepath):
            logging.info(f"📦 {gz_filename} ya existe. Saltando descarga."); continue
...
def fastqc(directory_fastq, output_dir, container_img):
    ...
    if len([f for f in os.listdir(output_dir) if f.endswith("_fastqc.html")]) >= len(fastq_files):
        logging.info("⏩ FastQC ya ejecutado para todos los archivos. Se omite."); return
    container_cmd = os.environ.get("APPTAINER_CMD", "singularity")
...
def run_trimmomatic(input_dir, output_dir, container_img, adapters_file, seq_type, threads):
    ...
    if seq_type == "paired-end":
        ...
        for sample, (r1, r2) in samples.items():
            out1, out2 = (os.path.join(output_dir, f"{sample}_{i}.trimmed.fastq.gz") for i in [1, 2])
            if os.path.exists(out1) and os.path.exists(out2):
                logging.info(f"⏩ Trimmomatic ya hecho para {sample}, saltando."); continue
            out1_unp, out2_unp = (os.path.join(output_dir, f"{sample}_{i}.unpaired.fastq.gz") for i in [1, 2])
            ...
            try:
                ...
                logging.error(f"❌ Error en Trimmomatic para {sample}: {e.stderr}"); exit(1)

def get_reference_files(reference_dir):
    """Obtiene las rutas de los archivos FASTA y GTF."""
    ...
    if seq_type == "paired-end":
        ...
        for sample, (r1_path, r2_path) in samples.items():
            ...
            if os.path.exists(final_bam_file):
                logging.info(f"⏩ El archivo BAM final para {sample} ya existe. Saltando alineamiento."); continue
...
def generate_count_matrix(bam_dir, gtf_file, output_file, container_img, seq_type, threads):
    ...
    if not bam_files: logging.warning("⚠️ No se encontraron archivos BAM."); return
    cmd_list = [container_cmd, "exec", container_img, "featureCounts", "-T", str(threads), "-a", gtf_file, "-o", output_file]
    if seq_type == "paired-end":
        logging.info("featureCounts en modo Paired-End."); cmd_list.extend(["-p"])
    else:
        logging.info("featureCounts en modo Single-End.")
...
def get_main_variable_from_formula(design_formula):
    """
    Extrae la variable principal de la fórmula de diseño de forma robusta.
    Funciona para fórmulas simples (~ condition) y complejas (~ var1 + var2).
    """
    try:
        # Paso 1: Elimina el '~' y los espacios de los extremos del texto completo.
        clean_formula = design_formula.replace("~", "").strip()
        
        # Paso 2: Divide la fórmula por el símbolo '+' y coge el último trozo.
        main_variable = clean_formula.split('+')[-1]
        
        # Paso 3: Limpia los espacios sobrantes del trozo final y lo devuelve.
        return main_variable.strip()
    except Exception:
        # Si algo inesperado falla, devuelve 'condition' como último recurso.
        return 'condition'
    
def generate_contrasts_from_metadata(metadata_path, control_group, design_formula):
    """Genera contrastes automáticamente leyendo la variable principal de la fórmula de diseño."""
    try:
        main_variable = get_main_variable_from_formula(design_formula)
        logging.info(f"Detectada la variable principal '{main_variable}' desde la fórmula de diseño para generar contrastes.")
        df = pd.read_csv(metadata_path)
        if main_variable not in df.columns:
            logging.error(f"❌ La columna '{main_variable}' (detectada de la fórmula de diseño) no se encuentra en el archivo de metadatos.")
            return None
        all_conditions = df[main_variable].unique()
        contrasts = [ [c, control_group] for c in all_conditions if c != control_group ]
...
def run_deseq2(config, base_dir, counts_file_path, alignments_dir, gtf_file):
    ...
    contrasts = generate_contrasts_from_metadata(
        deseq2_config.get("metadata_path"),
        deseq2_config.get("control_group"),
        deseq2_config.get("design_formula")
    )
    if not contrasts:
        logging.warning("⚠️ No se pudieron generar los contrastes. Saltando análisis DESeq2."); return
    ...
    # --- LÍNEAS MODIFICADAS ---
    # Leer la configuración del análisis funcional
    func_analysis_config = config.get("functional_analysis", {})
    run_kegg = func_analysis_config.get("run_kegg", False)
    # --- FIN DE LÍNEAS MODIFICADAS ---

    for i, (treatment_group, control_group_contrast) in enumerate(contrasts):
        logging.info(f"🚀 Ejecutando DESeq2 (Comparación {i+1}/{len(contrasts)}): {treatment_group} vs {control_group_contrast}")
        cmd = [
            os.environ.get("APPTAINER_CMD", "singularity"), "exec", images_config.get("r_deseq2"), "Rscript", scripts_config.get("r_script_path"),
            ...
            "--strip_gene_version", str(annotation_config.get("strip_gene_version", False)),
        ...
        # --- LÍNEAS MODIFICADAS ---
        # Añadir los argumentos de KEGG si está activado
        if run_kegg:
            cmd.extend([
                "--run_kegg", "TRUE",
                "--kegg_organism_code", func_analysis_config.get("kegg_organism_code", ""),
        ...
        try:
            ...
            if result.stderr: logging.warning(f"--- Mensajes de R (stderr) ---\n{result.stderr}")
        except subprocess.CalledProcessError as e:
...
def run_multiqc(base_dir, container_img):
    """Ejecuta MultiQC para generar un informe de calidad agregado."""
    logging.info("--- PASO 9: Generando informe con MultiQC ---")
    output_dir = os.path.join(base_dir, "MULTIQC_REPORT")
    create_directory(output_dir)
    container_cmd = os.environ.get("APPTAINER_CMD", "singularity")
    cmd = [container_cmd, "exec", container_img, "multiqc", base_dir, "-o", output_dir, "--force"]
    logging.info("🚀 Ejecutando MultiQC...")
    try:
        subprocess.run(cmd, check=True, capture_output=True, text=True)
...
def main():
    ...
    try:
        ...
        print(f"❌ No se encontró el archivo de configuración: {args.config}"); return
    except json.JSONDecodeError as e:
        print(f"❌ Error leyendo JSON: {e}"); return
    ...
    if not fasta_file or not gtf_file:
        logging.error("❌ No se encontraron archivos de referencia FASTA o GTF. Saliendo."); return
    ...
    if counting_method == "featurecounts":
        ...
        logging.error(f"❌ Método de conteo '{counting_method}' no reconocido."); return

    run_deseq2(config, base_dir, counts_file_path, alignments_dir, gtf_file)

    if images.get("multiqc"):
        validate_container(images.get("multiqc"), "multiqc")
        run_multiqc(base_dir, images.get("multiqc"))
